<?php
$query = "SELECT * FROM billing WHERE shop_url = '".$shopify->get_url()."' ";
...
if (isset($_GET['charge_id']) || $billing_data['charge_id']) {
    ...
    $query = array(
        "query" => '{
            node(id: "gid://shopify/AppPurchaseOneTime/' . $charge_id . '"){
    ...
    $query = "INSERT INTO  billing(shop_url, charge_id, gid, status) VALUE ('" .$shop_url. "', '" .$charge_id. "', '" .$gid. "', '" .$status. "') ON DUPLICATE KEY UPDATE status= '" . $status ."'";
    $connection->query($query);
} else {
    $query = array(
        "query" => 'mutation{
            appPurchaseOneTimeCreate(
                name: "Shopi One-Time Charge"
                price: { 
                    amount: 1.1, 
                    currencyCode: USD
                }
